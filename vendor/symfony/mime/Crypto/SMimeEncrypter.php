...
final class SMimeEncrypter extends SMime
{
     ...
     * @param int|null        $cipher      A set of algorithms used to encrypt the messages. Must be one of these PHP constants: https://www.php.net/manual/en/openssl.ciphers.php
     */
    ...
    public function encrypt(Message $message): Message
    {
        ...
        if (!@openssl_pkcs7_encrypt(stream_get_meta_data($bufferFile)['uri'], stream_get_meta_data($outputFile)['uri'], $this->certs, [], 0, $this->cipher)) {
            throw new RuntimeException(sprintf('Failed to encrypt S/Mime messages. Error: "%s".', openssl_error_string()));
        }
