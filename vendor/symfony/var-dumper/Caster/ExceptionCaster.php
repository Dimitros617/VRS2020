...
class ExceptionCaster
{
    ...
    private static function filterExceptionArray(string $xClass, array $a, string $xPrefix, int $filter): array
    {
        ...
        if (isset($a[Caster::PREFIX_PROTECTED.'messages']) && false !== strpos($a[Caster::PREFIX_PROTECTED.'messages'], "@anonymous\0")) {
            $a[Caster::PREFIX_PROTECTED.'messages'] = preg_replace_callback('/[a-zA-Z_\x7f-\xff][\\\\a-zA-Z0-9_\x7f-\xff]*+@anonymous\x00.*?\.php(?:0x?|:[0-9]++\$)[0-9a-fA-F]++/', function ($m) {
                return class_exists($m[0], false) ? (get_parent_class($m[0]) ?: key(class_implements($m[0])) ?: 'class').'@anonymous' : $m[0];
            }, $a[Caster::PREFIX_PROTECTED.'messages']);
        }
